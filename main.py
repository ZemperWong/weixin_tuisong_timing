...
def lucky():
    if (lucky_API!="替换掉我"):
        ...
        data = res.read()
        data = json.loads(data)
        data = str(data["newslist"][1]["content"])+"#"+str(data["newslist"][8]["content"])
        return data
    else:
        return ""
...
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips, note_en, note_ch, health_tip, lucky_):
    ...
    # 获取上次亲亲的日子的日期格式
    last_year = int(config["last_date"].split("-")[0])
    last_month = int(config["last_date"].split("-")[1])
    last_day = int(config["last_date"].split("-")[2])
    last_date = date(last_year, last_month, last_day)
    # 获取在一起的日期差
    love_days = str(today.__sub__(love_date)).split(" ")[0]
    # 获取上次亲亲的日期差
    last_days = str(today.__sub__(last_date)).split(" ")[0]
    # 获取所有生日数据
    birthdays = {}
    for k, v in config.items():
        if k[0:5] == "birth":
            birthdays[k] = v
    # 解析lucky数据
    love_per = str(lucky_.split("#")[0])
    xingzuo = "永远都是射手座！"
    gaishu = str(lucky_.split("#")[1])
